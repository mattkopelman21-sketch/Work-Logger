...
def add_note():
    d = request.json
    with get_db() as conn:
        cur = conn.execute("INSERT INTO notes (title,body,tag,date) VALUES (?,?,?,?)",
...
def update_note(nid):
    d = request.json
    with get_db() as conn:
        conn.execute("UPDATE notes SET title=?,body=?,tag=? WHERE id=?",
...
def add_task():
    d = request.json
    with get_db() as conn:
        cur = conn.execute("INSERT INTO tasks (title,done,due,priority,tag) VALUES (?,0,?,?,?)",
...
def update_task(tid):
    d = request.json
    with get_db() as conn:
        conn.execute("UPDATE tasks SET title=?,done=?,due=?,priority=?,tag=? WHERE id=?",
...
def get_customers():
    with get_db() as conn:
        ...
        result = []
        for c in customers:
            cnotes = conn.execute("SELECT * FROM customer_notes WHERE customer_id=? ORDER BY id DESC", (c["id"],)).fetchall()
            d = dict(c)
            d["cnotes"] = [dict(n) for n in cnotes]
            result.append(d)
...
def add_customer():
    d = request.json
    with get_db() as conn:
        cur = conn.execute("INSERT INTO customers (name,contact,phone,email,notes) VALUES (?,?,?,?,?)",
...
def add_customer_note(cid):
    d = request.json
    with get_db() as conn:
        cur = conn.execute("INSERT INTO customer_notes (customer_id,body,date) VALUES (?,?,?)",
            (cid, d["body"], str(date.today())))
        conn.commit()
        row = conn.execute("SELECT * FROM customer_notes WHERE id=?", (cur.lastrowid,)).fetchone()
    return jsonify(dict(row)), 201

@app.route("/api/customers/<int:cid>/notes/<int:nid>", methods=["DELETE"])
def delete_customer_note(cid, nid):
    with get_db() as conn:
        conn.execute("DELETE FROM customer_notes WHERE id=? AND customer_id=?", (nid, cid))
        conn.commit()
    return jsonify({"ok": True})

HTML = open(os.path.join(os.path.dirname(__file__), "index.html")).read() if os.path.exists(os.path.join(os.path.dirname(__file__), "index.html")) else ""
...
async function api(method,url,body){const opts={method,headers:{'Content-Type':'application/json'}};if(body)opts.body=JSON.stringify(body);const r=await fetch(url,opts);return r.json();}
